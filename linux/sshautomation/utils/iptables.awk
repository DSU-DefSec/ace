...
#region bash
sh -c "ss -napOH4 | awk -f $BASH_SOURCE" {0..0}
"exit" {0..0}
#endregion bash
...
$2 in INBOUND_CONNECTION_TYPES_ARRAY {
  ...
  # Log
  printLog("INFO", "New inbound rule - " colored("yellow", sprintf("%5d", local[2])) "/" $1 " (" colored("blue", name) ")");
...
$2 in OUTBOUND_CONNECTION_TYPES_ARRAY {
  ...
  printLog("INFO", "New outbound rule - " sprintf("%15s", remoteIP) ":" colored("yellow", sprintf("%5d", remote[2])) "/" $1 " (" colored("blue", name) ")");
